...
- **Capability echo:** project link/re-link now reads the effective `capabilities` echoed by Burrow (exposed by `useburrow/sdk-php` 0.9.10) and persists the authoritative `ecommerce_funnel` value, which gates the headless-Shopify collector. With older SDK versions the plugin falls back to the locally persisted capability; the composer constraint stays `^0.9.9`.
